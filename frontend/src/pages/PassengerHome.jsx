  ...
  useEffect(() => {
    ...
    if (pickupId && destinationId) {
      ...
      if (p && d) {
        ...
        const routeChanged = lastRouteRef.current.pickupId !== pickupId || lastRouteRef.current.destinationId !== destinationId;
        if (routeChanged) {
    ...
    drivers.forEach(driver => {
      ...
      if (existing) {
        existing.setLatLng(driver.coords);
      } else {
...
export default function PassengerHome({ user, onLogout }) {
  ...
  useEffect(() => {
    // Keep the online driver list fresh on location updates as well
  ...
  useEffect( () =>{
    ...
    const id = setInterval(syncRideData,2000);
            ...
            <div style={{
              ...
              {pickup && destination && (
                <div style={{
                  ...
                  <div>
                    <p style={{
                      margin: 0, fontSize: 11, color: "#888", textTransform: "uppercase",
